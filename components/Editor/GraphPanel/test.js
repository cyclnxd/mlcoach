...
import Paramaters from "./Variables";
          ...
          localforage.getItem(id).then((file) => {
            if (file !== undefined && file !== null) {
              ...
              unstable_batchedUpdates(() => {
                ...
                setRender(true);
                console.log(params.getAxisParams);
              });
            }
          });
        } else {
      ...
      >
        {store.getState().clickedNode !== -1 ? (
          <PlotGraph
            ...
            chartParams={params.getChartParams}
            axisParams={params.getAxisParams}
            update={render}
            axisChanged={params.getReRender}
            style={{
              width: "100%",
              height: "100%",
              position: "absolute",
...
function GraphEditor({ onClose, columns }) {
  const handleXAxisChange = (event, value) => {
    params.setXAxis(value);
    params.setReRender;
  };
  const handleYAxisChange = (event, value) => {
    params.setYAxis(value);
    params.setReRender;
  };
  return (
